...
object Test {
  ...
  object sum2 extends FA1[TreapMap[Int,Int],Int] {
    def apply(t: F0[TreapMap[Int,Int]]) = {
      ...
      if (l.isEmpty) {
        ...
        else tt.value ++ %(sum2,tt.right)
      }
      else {
        if (r.isEmpty) %(sum2,tt.left) ++ tt.value
        else %(sum2,tt.left) ++ tt.value ++ %(sum2,tt.right)
      }
    }
  ...
  final def main(args: Array[String]): Unit = {
    ...
    var e1: TreapSet[Int] = empty
    var e2: TreapSet[Int] = empty

    var i1 = 0
    var i2 = 5

    while (i1 < 10) {
      e1 = e1.put(SetEntry(i1))
      e2 = e2.put(SetEntry(i2))
      i1 = i1 + 1
      i2 = i2 + 1
    }

    val op = ICombinator(Intersect())
    val e = %(fac,10)
    //val e = ('a~1 ++ 2) ** 'c~(3 ++ 4)
