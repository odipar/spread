<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="f1" mode="node">
        <xsl:call-template name="ellipse">
            <xsl:with-param name="label" select="function/@name"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="f2" mode="node">
        <xsl:call-template name="ellipse">
            <xsl:with-param name="label" select="function/@name"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="f3" mode="node">
        <xsl:call-template name="ellipse">
            <xsl:with-param name="label" select="function/@name"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="f1" mode="edge">
        <section name="edge">
            <attribute key="source" type="int"><xsl:value-of select="../@id"/></attribute>
        ...
        <section name="edge">
            <attribute key="source" type="int"><xsl:value-of select="../@id"/></attribute>
            <attribute key="target" type="int"><xsl:value-of select="reduce/@id"/></attribute>
            <attribute key="label" type="String">red</attribute>
        </section>
        </xsl:if>
    </xsl:template>

    <xsl:template match="f2" mode="edge">
        <section name="edge">
            <attribute key="source" type="int"><xsl:value-of select="../@id"/></attribute>
            <attribute key="target" type="int"><xsl:value-of select="arg1/@id"/></attribute>
            <attribute key="label" type="String">arg1</attribute>
    ...
    <xsl:template match="f3" mode="edge">
        <section name="edge">
            <attribute key="source" type="int"><xsl:value-of select="../@id"/></attribute>
            <attribute key="target" type="int"><xsl:value-of select="arg1/@id"/></attribute>
            <attribute key="label" type="String">arg1</attribute>
